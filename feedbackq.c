...
void handle_instruction(Instruction_t *instruction, int tick) {
	...
	if(instruction->burst_time == 0) { 
        ...
        struct Task task; //create a new task variable and initialize all of its variables
        task.id = task_id; 
        task.burst_time = 0;
        ...
        task.next = NULL;
		task_table[task_id] = task; //insert the task into the task_table at the index of the task_id
        ...
        turn_around_time = task_table[task_id].total_wait_time + task_table[task_id].total_execution_time;
        ...
        if(task_table[task_id].current_queue == 0){ //If the task is not on a queue and gets burst_time then add it to the queue_1 
...
Task_t *peek_priority_task() {
	// TODO ... Insert your Code Here
    if(!is_empty(queue_1)){ //return the head of queue_1 if queue_1 is not empty
        return queue_1->start;
    } else if (!is_empty(queue_2)){//return the head of queue_2 if queue_2 is not empty
        return queue_2->start;
    } else if (!is_empty(queue_3)){ //return the head of queue_3 if queue_3 is not empty
...
void boost(int tick) {
    ...
    while(!is_empty(queue_3)){ //While queue_3 is not empty remove all the tasks of queue_3 and add them to queue_1
        queue_3->start->current_queue = 1;
        enqueue(queue_1, dequeue(queue_3));
    }

    while(!is_empty(queue_2)){//While queue_2 is not empty remove all the tasks of queue_2 and add them to queue_1
        queue_2->start->current_queue = 1;
...
void scheduler() {
    ...
    if(priority_task == NULL){ //if there is nothing in any of the priority queues then don't change the current task
        if(remaining_quantum == 0 && current_task != NULL){ //if the task is out of quantum
    ...
    if(current_task != NULL && remaining_quantum > 0 && current_task->remaining_burst_time > 0 && priority_task->current_queue >= current_task->current_queue){ //If the current task has more quantum, more burst_time, and priority then current_task stays the same
        return;
    }
    
    if (current_task != NULL && remaining_quantum == 0 && current_task->remaining_burst_time > 0){ //If the current task has no quantam_remaining, then decrease its task level and if it is still the highest priority then don't change the priority task
        decrease_task_level(current_task);
    ...
    dequeue(get_queue_by_id(priority_task->current_queue));//remove the priority task from its queue and set the priority task to the current task
